...
def _interpolate_reach_bank_elevation_surface(
    ...
    if not np.isfinite(reach_grade):
        reach_grade = MIN_SLOPE
    reach_grade = float(np.clip(reach_grade, MIN_SLOPE, MAX_SLOPE))
...
def _finalize_cross_section_records(
    ...
    """
    # Set baseline channel roughness before smoothing the bank elevations.
...
def run_main_loop(
    ...
    # data, this flag is used to determine whether to collect that data during the main loop.
    want_xs = bool(
