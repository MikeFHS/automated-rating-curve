...
| `Build_Representative_Cross_Section` | False | bool | When true, ARC builds a representative cross section for each positive reach ID in `Stream_File`. ARC samples and preprocesses each stream-cell cross section, applies bathymetry only when a bathymetry output path is configured, and recomputes area, wetted perimeter, velocity, discharge, and top width every 0.10 m above each local thalweg. Each reach is capped at 25 m, or at the last successful stage before any hydraulic value becomes non-finite. The CSV stores reach-median stream slope, reach-median hydraulics, and representative dimensions derived from monotonic staged top width and area. `Representative_Cross_Section_File` is required; `Flow_File_QMax` is not. |
