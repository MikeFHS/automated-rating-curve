...
| `Flow_File_BF` | --- | str | Column name in the flow file that contains baseflow or bankfull discharge values used by the downstream-to-upstream reach-network energy solver. The initial network depths are subsequently reduced to inclusive interquartile-filtered reach medians and constrained to stay equal or increase downstream. This field is optional only when the full drainage-area bathymetry parameter set is provided (`drainage_area_field`, `coefficient_depth`, `exponent_depth`, `coefficient_width`, and `exponent_width`). |
