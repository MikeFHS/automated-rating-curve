...
| `Bathy_Use_Banks` | False | bool | When false, ARC assumes that the DEM is representative of the water surface (typically not in a flood stage, and often much less than bankfull), and that bathymetry depth should be estimated within the detected banks using either `Flow_File_BF` or the optional drainage-area power-law depth. When true, ARC assumes that the detected bank elevations represent bankfull conditions and is free to recreate the bathymetry in the channel using either the discharge-based or drainage-area-based depth target. In both cases ARC samples every cross section first, evaluates the land-cover/INFLECT/local-DEM bank hierarchy across the cached sections, filters reach-scale top-width outliers, and fills invalid bank indices with the reach-median top width. ARC then establishes network outlet controls and walks each reach upstream-to-downstream. An observed cell minimum below the active interpolation becomes a new anchor; ARC refits the slope from the nearest upstream anchor to that low point, then resets the outgoing grade toward the fixed reach outlet. Every cell-to-cell segment is constrained to at least `MIN_SLOPE`, and an outlet-feasibility floor prevents extrapolation beneath the network control. The resulting per-cell elevation and outgoing grade become the vertical bathymetry control and hydraulic slope while the local width geometry is retained. See [**Bathymetry**](bathymetry.md) for more details. |
