...
If you also want a reach-level geometry summary, set `Build_Representative_Cross_Section: True` and provide `Representative_Cross_Section_File`. ARC will then build a representative cross-section CSV directly from the sampled cross sections for each `Flow_File_ID`. The run begins by sampling, low-spot-adjusting, and resampling every stream-cell cross section, then evaluating the reach-scale INFLECT and bank-finding workflow across the cached sections, then replacing reach-scale top-width outliers and filling any remaining invalid bank indices with the reach-median top width. ARC next smooths bank elevations upstream-to-downstream: an observed cell minimum below the active interpolation becomes a new anchor, the interval from the nearest upstream anchor is refitted to that low point, and the outgoing grade is reset toward the fixed reach outlet. Every segment retains at least `MIN_SLOPE`, and accepted anchors cannot force downstream extrapolation beneath the outlet control. Only then does ARC apply requested bathymetry. Instead of using `s_flow_file_qmax` to define the representative maximum WSE, ARC uses the reach-average `get_representative_inflect_curve()` result to define the flood-terrace depth for the reach, recomputes Manning hydraulics every 0.01 m above each sampled thalweg up to that terrace depth, and stores the reach-median hydraulic stage data plus the representative dimensions derived from staged top width and staged area.
