...
def Thin_GeoJSON_data(curve_data_gdf):
    """
    Thins the data being output to the GeoJSON file by removing stream cells within 50 meters that are within 0.05% of one another.
...
def Write_GeoJSON_File(OutGeoJSON_File, OutProjection, curve_data_gdf, Thin_GeoJSON):
    """
    ...
    curve_data_gdf = curve_data_gdf[(curve_data_gdf['COMID_MedDEP']>0) & (curve_data_gdf['CP_DEP']<3.0*curve_data_gdf['COMID_MedDEP']) & (curve_data_gdf['CP_DEP']!="1")]

    # Filter the Geodataframe the GeoDataFrame
    if Thin_GeoJSON is True:
        curve_data_gdf = Thin_GeoJSON_data(curve_data_gdf)
...
def Run_Main_REDUCED_Curve_to_GEOJSON_Program(WatershedName, CurveParam_File, COMID_Q_File, STRM_Raster_File, DEM_Raster_File, OutGeoJSON_File, OutProjection, SEED_Lat, SEED_Lon, SEED_COMID, SEED_r, SEED_c, SEED_MinElev, SEED_MaxElev, Thin_GeoJSON):
    """
...
if __name__ == "__main__":
    ...
    for WatershedName in Watershed_List:
        ...
        for scenario in scenarios:
            ...
            Run_Main_REDUCED_Curve_to_GEOJSON_Program(WatershedName, CurveParam_File, COMID_Q_File, STRM_Raster_File, DEM_Raster_File, OutGeoJSON_File, OutProjection, SEED_Lat, SEED_Lon, SEED_COMID, SEED_r, SEED_c, SEED_MinElev, SEED_MaxElev, Thin_GeoJSON)
