...
def get_xs_index_values(ia_xc_dr_index_main: np.ndarray, ia_xc_dc_index_main: np.ndarray, ia_xc_dr_index_second: np.ndarray, ia_xc_dc_index_second: np.ndarray, da_xc_main_fract: np.ndarray,
...
def sample_cross_section_from_dem(da_xs_profile: np.ndarray, i_row: int, i_column: int, dm_elevation: np.ndarray, i_center_point: int, ia_xc_row_index_main: np.ndarray,
...
def adjust_profile_for_bathymetry(da_xs_profile: np.ndarray, i_bank_index: int, d_total_bank_dist: float, d_trap_base: float, d_distance_z: float, d_distance_h: float, d_y_bathy: float,
                                  d_y_depth: float, dm_output_bathymetry: np.ndarray, ia_xc_r1_index_main: np.ndarray, ia_xc_c1_index_main: np.ndarray):
    """
    ...
    # If banks are calculated, make an adjustment for to the depth
    if i_bank_index > 0:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        MIF_Name = "/Users/dloney/Library/CloudStorage/OneDrive-Personal/Documents/work/follum/projects/automated_rating_curve_generator/TestModel/AutoRoute_InputFiles/AUTOROUTE_INPUT_CO_06759500_Bathymetry.txt"
    ...
    i_row_top = nrows + i_boundary_number
    i_column_bottom = i_boundary_number
    i_column_top = ncols + i_boundary_number
    ...
    for i_entry_cell in range(i_number_of_stream_cells):
        ...
        if i_entry_cell == da_percent_cells[i_counter]:
            print('  ' + str(int((da_percent_cells[i_counter] + 1) * 100 / i_number_of_stream_cells)) + ' percent of cells completed')
        ...
        d_distance_z = get_xs_index_values(ia_xc_dr_index_main, ia_xc_dc_index_main, ia_xc_dr_index_second, ia_xc_dc_index_second, da_xc_main_fract, da_xc_second_fract, d_xs_direction, i_row_cell,
        ...
        xs1_n = sample_cross_section_from_dem(da_xs_profile1, i_row_cell, i_column_cell, dm_elevation, i_center_point, ia_xc_r1_index_main, ia_xc_c1_index_main, i_row_cell + ia_xc_dr_index_second,
        ...
        xs2_n = sample_cross_section_from_dem(da_xs_profile2, i_row_cell, i_column_cell, dm_elevation, i_center_point, ia_xc_r2_index_main, ia_xc_c2_index_main, i_row_cell + ia_xc_dr_index_second * -1,
        ...
        if i_low_spot_range > 0:
            i_lowest_point_index_offset, d_low_point_elev = adjust_cross_section_to_lowest_point(i_lowest_point_index_offset, d_low_point_elev, da_xs_profile1, da_xs_profile2, ia_xc_r1_index_main,
                                                                                                 ia_xc_r2_index_main, ia_xc_c1_index_main, ia_xc_c2_index_main, xs1_n, xs2_n, i_center_point)
        ...
        if d_increments > 0:
            for d_entry_angle_adjustment in l_angles_to_test:
                ...
                d_distance_z = get_xs_index_values(ia_xc_dr_index_main, ia_xc_dc_index_main, ia_xc_dr_index_second, ia_xc_dc_index_second, da_xc_main_fract, da_xc_second_fract, d_xs_angle_use,
        ...
        if d_q_baseflow > 0.0:
            ...
            if i_total_bank_cells > 1:
                adjust_profile_for_bathymetry(da_xs_profile1, i_bank_1_index, d_total_bank_dist, d_trap_base, d_distance_z, d_h_dist, d_y_bathy, d_y_depth, dm_output_bathymetry, ia_xc_r1_index_main, ia_xc_c1_index_main)
                adjust_profile_for_bathymetry(da_xs_profile2, i_bank_2_index, d_total_bank_dist, d_trap_base, d_distance_z, d_h_dist, d_y_bathy, d_y_depth, dm_output_bathymetry, ia_xc_r1_index_main, ia_xc_c1_index_main)
